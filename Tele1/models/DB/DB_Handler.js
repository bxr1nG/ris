...
let usemain = `use RISmain;`;
let use = `use RIS${DBid - 1};`;
...
class DB {
	...
	getbyId(id) {
		return connectionPool.then(pool =>
			pool.query(`${use} SELECT * FROM Watts where id = ${id}`)
		);
	}
	...
	getMaxDate(id) {
		return connectionPool.then(pool =>
			pool.query(
				`${use} select max(date) as [date] from Watts where id = ${id}`
			)
		);
	}
	...
	getFromDate(id, date) {
		return connectionPool.then(pool =>
			pool.query(
				`${use} SELECT * FROM Watts where id = ${id} and date > '${date}'`
			)
		);
	}
	...
	getService() {
		return connectionPool.then(pool =>
			pool.query(`${use} SELECT DISTINCT id FROM Watts`)
		);
	}
	...
	getAll() {
		return connectionPool.then(pool =>
			pool.query(`${use} SELECT * FROM Watts`)
		);
	}
	...
	insert(body) {
		return connectionPool.then(pool => {
			...
			let command = `${usemain} INSERT INTO Watts values `;
			body.forEach(fields => {
				command += '(';
